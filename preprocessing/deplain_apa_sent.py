...
output_json_path = 'deplain-apa-simple.json'

# Read the Excel file
df = pd.read_csv(input_xlsx_path)

# Select only the "cefr" and "text_corrected" columns
filtered_df = df[['simplification', 'language_level_simple', 'license', 'simple_title']]

# Transform the DataFrame into the desired JSON format
transformed_data = []

for _, row in filtered_df.iterrows():
    cefr_level = row['language_level_simple'] if pd.notna(row['language_level_simple']) else 'Unknown'  # Default to 'Unknown' if missing
    text_content = row['simplification'] if pd.notna(row['simplification']) else ''  # Default to empty string if missing
    license = row['license'] if pd.notna(row['license']) else ''
    title = row['simple_title'] if pd.notna(row['simple_title']) else ''

    transformed_entry = {
        "title": str(title.strip()),  # Assuming 'na' as per your example
        "lang": "de",  # Assuming English language
        "source_name": "deplain-apa-sent",
        "format": "sentence-level",  # Assuming paragraph-level format
        "category": "reference",
